...
**目的を定める力、文脈を渡す力、結果を見極める力、必要なときに軌道修正する力、AI環境を自分で進化させる力**——エンジニアに限らず、あらゆるAIユーザーの協働スタイルを診断する。
...
> **注意**: このプラグインは一晩で作ったプロトタイプです。評価フレームワークは今後も継続的に洗練していきます。スコアはあくまで参考値であり、対話ログからの推定には本質的な限界があります。「AIを使うのがうまいとは何か」に対する一つの切り口として、気軽に試してみてください。

### 思想

- **対話の構造と結果で判断する。** 特定のフレーズやテクニックの有無は見ない。
- **介入の量ではなく、質と適切さを見る。** 上流（目的・文脈）が優れていれば操舵や検証の必要は減る。それは能力の不足ではなく、正しい進化。
- **AIの進化を前提とする。** ツールの機能は変化し、かつてのテクニックが標準機能になることもある。評価はツールに依存しない抽象的な能力を見る。
...
| **検証・採択力** | 結果を見極め、自分の判断で採否を決める力 |
| **対話操舵力** | 必要なときに的確に軌道修正する力（補償スキル） |
| **学習資産化力** | うまくいった進め方を再利用可能な資産に変える力 |

上流の設計力（目的・文脈）が最もレバレッジが高く、操舵力は上流が機能しなかったときの補償スキルとして位置づけている。

### 統制レイヤー（条件付き評価）

高リスク・不可逆・外部影響のある場面でのみ評価。委任範囲の設計、不可逆操作前の確認点を見る。
...
6. **AI協働プロフィール** — 5軸のスコア・confidence・エビデンス引用
7. **統制レイヤー所見** — 条件付き評価
...
│       ├── SKILL.md               # スキル定義（実行手順）
│       └── references/
│           ├── evaluation-framework.md  # 5軸+統制レイヤー評価基準
